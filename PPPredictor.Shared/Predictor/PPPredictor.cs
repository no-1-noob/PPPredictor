...
namespace PPPredictor.Shared.Predictor
{
    internal class PPPredictor : IPPPredictor
    {
        ...
        private Core.DataType.BeatSaberEncapsulation.GameplayModifiers _gameplayModifiers = new Core.DataType.BeatSaberEncapsulation.GameplayModifiers();
        ...
        public PPPredictor(Leaderboard leaderBoard, CalculatorInstance calculatorInstance)
        {
            ...
            int index = 0;
            if(PluginBase.ProfileInfo.MapPoolSelection.TryGetValue(leaderboardName.ToString(), out string value))
            {
        ...
        public object CurrentMapPool
        {
            ...
            set
            {
                ...
                {
                    PluginBase.ProfileInfo.MapPoolSelection[leaderboardName.ToString()] = currentMapPool.Id;
                }
        ...
        public void ChangeGameplayModifiers(Core.DataType.BeatSaberEncapsulation.GameplayModifiers gameplayModifiers)
        {
            if (currentMapPool != null && gameplayModifiers != null)
            {
                _gameplayModifiers = gameplayModifiers;
                _currentBeatMapInfo = calculatorInstance.ApplyModifiersToBeatmapInfo(leaderboardName, currentMapPool.Id, _currentBeatMapInfo, gameplayModifiers);
                _currentBeatMapInfo.MaxPP = -1;
                CalculatePP();
            }
        }
        
        public async void DifficultyChanged(PPPBeatMapInfo beatMapInfo)
        {
            await UpdateCurrentBeatMapInfos(beatMapInfo);
        }
        
        public async Task UpdateCurrentBeatMapInfos(PPPBeatMapInfo beatMapInfo)
        {
            _currentBeatMapInfo = beatMapInfo;
        ...
        public PPPBeatMapInfo GetModifiedBeatMapInfo(Core.DataType.BeatSaberEncapsulation.GameplayModifiers gameplayModifiers, bool levelFailed = false, bool levelPaused = false)
        {
            PPPBeatMapInfo pppBeatMapInfo = calculatorInstance.ApplyModifiersToBeatmapInfo(leaderboardName, currentMapPool.Id, _currentBeatMapInfo, gameplayModifiers, levelFailed, levelPaused);
        ...
        public bool IsRanked()
        {
            ...
            {
                PluginBase.ErrorPrint($"IsRanked {ex.Message}");
                return false;
            }
        }
        ...
        private async Task DisplaySession(bool doResetSession)
        {
            ...
            {
                if (PluginBase.ProfileInfo.DisplaySessionValues)
                {
        ...
        public async Task GetMapPoolIconData()
        {
            ...
            {
                PluginBase.ErrorPrint($"GetMapPoolIconData {MapPoolIcon} Error: {ex.Message}");
            }
        }
