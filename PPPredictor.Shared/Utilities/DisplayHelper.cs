...
namespace PPPredictor.Utilities
{
    class DisplayHelper
    {
        ...
        public static string GetDisplayColor(double value, bool invert, bool isPPGainWithVergeOption = false)
        {
            ...
            else if (isPPGainWithVergeOption && PluginBase.ProfileInfo.PpGainCalculationType == PPGainCalculationType.Raw && value < 0 && value >= PluginBase.ProfileInfo.RawPPLossHighlightThreshold)
            {
                return ColorYellow;
            }
            else if (value < 0)
        ...
        public static byte[] ResizeImage(byte[] data, int width, int height)
        {
            ...
            {
                PluginBase.ErrorPrint($"ResizeImage error {ex.Message}");
            }
            return output;
        }
    }
