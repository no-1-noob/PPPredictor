            ...
            try
            {
                if (beatMapInfo.SelectedCustomBeatmapLevel != null)
                {
                    ...
                    if (cachedInfo == null || refetchInfo)
                    {
                        ...
                        if (song != null)
                        {
                            BeatLeaderDifficulty diff = song.difficulties.FirstOrDefault(x => x.value == ParsingUtil.ParseDifficultyNameToInt(beatMapInfo.BeatmapKey.difficulty.ToString()) && x.modeName.ToUpper() == beatMapInfo.BeatmapKey.beatmapCharacteristic.serializedName.ToUpper());
                            if (diff != null)
                            {
